...
$('.update-student').click(function () {
    ...
    $('#edit-student input[name="id"]').attr('value', id);

    var name = $(this).parent().siblings('td:nth-of-type(2)').text();
    $("#edit-student input[name='name']").attr('value', name);

    var department = $(this).parent().siblings('td:nth-of-type(3)').text();
    $("#edit-student select[name='department_id'] option[value='" + department + "']").attr('selected', 'selected');

    var email = $(this).parent().siblings('td:nth-of-type(4)').text();
    $("#edit-student input[name='email']").attr('value', email);

    var gender = $(this).parent().siblings('td:nth-of-type(5)').text();
    $("#edit-student select[name='gender']").attr('value', gender);

    var birthday = $(this).parent().siblings('td:nth-of-type(6)').text();
    $("#edit-student input[name='birthday']").attr('value', birthday);

    var address = $(this).parent().siblings('td:nth-of-type(7)').text();
    $("#edit-student input[name='address']").attr('value', address);

    var phone = $(this).parent().siblings('td:nth-of-type(8)').text();
    $("#edit-student input[name='phone']").attr('value', phone);

    $("#edit-student button[type='submit']").attr('disabled', true);
    $('#edit-student input, #edit-student select').on('change keyup', function () {
        $("#edit-student button[type='submit']").attr('disabled', false);
    });
});
